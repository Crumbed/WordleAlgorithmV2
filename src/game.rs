...
impl GameState {
    pub fn new() -> GameState {
        ...
        let mut this = GameState { 
            ...
            pos_ans: LinkedList::new(),
            guess: 1
        }.calc_weights();
        let mut words: Vec<Word> = WORDS.iter().map(|&word| {
            let bytes = word.as_bytes();
            let score = bytes.windows(2).map(|combo| {
                this.patterns.get(combo).unwrap_or(&0.0)
            }).sum::<f64>() + this.single[i![bytes[0]]][FL] + this.single[i![bytes[4]]][LL];

            Word { word, score }
        }).collect();
        words.sort_by(|a, b| a.score.partial_cmp(&b.score).unwrap());
        for word in words {
            this.pos_ans.push_front(word);
        }
        this.pos_ans.push_front(Word { word: "salet", score: 10. });

        return this;
    }

    fn calc_weights(mut self) -> GameState {
        let mut fl_count = [0; 27];
        ...
        WORDS.iter().for_each(|&word| {
            ...
            for b in 0..bytes.len() - 1 {
                ...
                *combo_count.entry(combo).or_insert(0) += 1;
            }

            let [fl, _, _, _, ll] = bytes else {
                panic!("huh??");
            };
    ...
    pub fn get_guess(&mut self, bytes: &[u8]) -> &str {
        if self.guess == 1 {
            self.guess += 1;
            return self.pos_ans.front().unwrap().word;
        }

        let last_guess = self.pos_ans.front().unwrap().word.as_bytes();
        let mut filter = [FilterNode::None(b' '); 5];
        for (i, b) in bytes.iter().enumerate() {
            let lastb = last_guess[i];
            filter[i] = match *b {
                b'b' => FilterNode::None(lastb),
                b'g' => FilterNode::Here(lastb, i),
                b'y' => FilterNode::NotN(lastb, i),
